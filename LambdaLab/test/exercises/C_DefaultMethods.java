...
public class C_DefaultMethods {
    ...
    public void c03_upcaseAllWords() {
        ...
        list.replaceAll(word -> word.toUpperCase(Locale.ROOT));
...
//        map.entrySet().forEach(entry -> entry.getValue().append(entry.getKey()));
...
//        map.entrySet().forEach(entry -> entry.setValue(entry.getValue() + entry.getKey()));
...
//        keys.forEach(key -> map.computeIfAbsent(key, (it) -> ""));
...
//                .forEach(entry->entry.setValue(entry.getKey()));
...
//        map.replaceAll((key, value) -> value.toUpperCase(Locale.ROOT));
//        keys.forEach(key -> map.putIfAbsent(key, key));
